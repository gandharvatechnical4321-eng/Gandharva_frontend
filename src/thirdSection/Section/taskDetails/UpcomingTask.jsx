...
const UpcomingTask = () => {
  ...
  const upcomingTasks = useSelector((state) => state.tasks.UpcomingTask) || [];
  const selectedTask = useSelector((state) => state.tasks.selectedTask);
  const reloadInterval = useSelector((state) => state.tasks.reloadInterval);

  const [filterStatusU, setFilterStatusU] = useState("");
  ...
  const fetchUpcomingTasks = async (page = 1, search = "") => {
    try {
      const resp = await axios.get(
        `${process.env.REACT_APP_BACKEND_URL}/api/task/client-deadline/after-now`,
        {
          params: {
            page,
            limit,
      ...
      if (resp?.data) {
        dispatch(setUpcomingTask(resp.data.data || []));
  ...
  useEffect(() => {
    if (selectedTask === "Upcoming Task") {
      fetchUpcomingTasks(currentPage, commanSearch);
    }
  ...
  useEffect(() => {
    if (selectedTask === "Upcoming Task") {
      setCurrentPage(1);
      fetchUpcomingTasks(1, commanSearch);
    }
    ...
    <section className="flex h-full min-h-0 w-full flex-col overflow-hidden bg-white">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative">
            <button
              ...
              title={filterStatusU ? "Filter active" : "Filter upcoming tasks"}
            >
              {!filterStatusU ? (
      ...
      <div className="min-h-0 flex-1 overflow-y-auto bg-gray-200 px-3 py-3">
        {upcomingTasks?.length > 0 ? (
          ...
          <div className="flex h-full items-center justify-center rounded-2xl border border-dashed border-slate-200 bg-gray-200 p-6 text-center">
            <div>
