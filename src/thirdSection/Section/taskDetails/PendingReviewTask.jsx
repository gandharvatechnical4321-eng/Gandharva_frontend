...
const isTutorPaymentHold = (task) => {
  ...
  return String(paymentStatus).toLowerCase() === "hold";
};

const isTutorDeadlineCrossed = (task) => {
  if (!task?.tutorDeadline) return false;

  const deadlineDate = new Date(task.tutorDeadline);
...
const isPendingReviewTask = (task) => {
  const statusMatched = REVIEW_STATUSES.includes(task?.status);
  const paymentHold = isTutorPaymentHold(task);
  const deadlineCrossed = isTutorDeadlineCrossed(task);

  return paymentHold && (statusMatched || deadlineCrossed);
};
...
const PendingReviewTask = () => {
  ...
  const pendingReviewTasks = useMemo(() => {
    let list = tasks.filter(isPendingReviewTask);
  ...
  const totalPages = Math.max(1, Math.ceil(pendingReviewTasks.length / limit));

  const paginatedTasks = useMemo(() => {
    const start = (currentPage - 1) * limit;
    return pendingReviewTasks.slice(start, start + limit);
  }, [pendingReviewTasks, currentPage, limit]);
      ...
      <div className="shrink-0 border-b border-slate-100 bg-white px-3 py-3">
          ...
          <div className="flex min-w-0 items-center gap-2">
            ...
            <div className="min-w-0">
              ...
              <p className="truncate text-xs font-bold text-slate-500">
                Tutor payment hold, review status, or tutor deadline crossed
              </p>
            </div>
          </div>
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder="Search pending review tasks..."
            className="h-10 w-full rounded-2xl border border-slate-200 bg-slate-50 px-3 pr-9 text-xs font-bold text-slate-700 outline-none transition placeholder:text-slate-400 focus:border-orange-300 focus:bg-white focus:ring-4 focus:ring-orange-50"
          />
        </div>
      </div>

      {/* Count Strip */}
      <div className="shrink-0 border-b border-slate-100 bg-orange-50/70 px-3 py-2">
        <p className="text-xs font-black text-orange-600">
          {pendingReviewTasks.length} pending review task
          {pendingReviewTasks.length === 1 ? "" : "s"} found
        </p>
      </div>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto bg-slate-50 px-3 py-3">
        {loading ? (
          ...
          <div className="flex h-full flex-col items-center justify-center rounded-3xl border-2 border-dashed border-slate-200 bg-white p-6 text-center">
            ...
            <p className="mt-1 max-w-[260px] text-xs font-semibold text-slate-400">
              No task currently matches tutor payment hold with review status or
              crossed tutor deadline.
            </p>
          </div>
        )}
      </div>
...
export default PendingReviewTask;